#!/usr/bin/env python3
"""关键帧事件范围界定 + 梯形权重 (配置驱动) —— 供 stage_label / frame_weight_trapezoid 共用。
...
(默认 configs/keyframe_weight_config.json), 不写死在代码里。流程分三步:
  ...
  2. 事件范围界定: 按任务把每个周期的关键节点(hold_start/hold_end)归为具体事件
     (抓/放/插入...), 每个事件有参考帧 t0 和一组窗口参数 (见下)。角色判定见
     `_role_cycle_map` (fill 持筒臂/笔臂、plug 末次插入、stack 双臂)。
  3. 权重赋值: 对每个事件 t0 生成非对称梯形权重窗, 逐帧取各事件窗口的 max; 逐帧
     关键帧标签 = 命中窗口的事件 key 集合 ('|' 连接, 无则 'none')。
...
配置 schema (configs/keyframe_weight_config.json):
  ...
  role  : 事件绑定到哪些周期 (fill: holder/pen; plug: all_hs/noninsert_he/insert_he;
          stack: all_hs/all_he), 语义见配置文件 _comment;
...
def _role_cycle_map(ti: int, state: np.ndarray, per_side: dict):
    ...
    if ti == 0:                                     # fill: holder / pen
        ...
        role_cycles = {
            "all_hs": all_c,
            "noninsert_he": [(s, c) for (s, c) in all_c
...
def episode_weight_and_labels(ti: int, state: np.ndarray, frame: np.ndarray,
                              detect_kwargs: dict, config: dict) -> dict:
    """单 episode -> 逐帧权重 + 逐帧关键帧标签。

    返回 {"weight":np.ndarray, "labels":list[str], "instances":[...],
          "meta":str, "per_side":...}。
    ...
    weight = np.full(T, WEIGHT_BASE, dtype=float)
    for inst in instances:
        weight = np.maximum(weight, trapezoid_weight(inst["t0"], frame, inst))

    # 逐帧标签: 事件范围 [a,b] 内的帧打上该 key, 多标签按配置顺序 '|' 连接
    ...
    return {"weight": weight, "labels": labels, "instances": instances,
