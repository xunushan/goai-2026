...
`Dexora_1B` is the XPolicyLab/RoboDojo adapter for the corresponding policy. It keeps integration-facing scripts at this directory level and leaves the original or vendored implementation in the nested source tree when present.
...
| `dexora_root` | Runtime or checkpoint option consumed by this adapter. |
...
| `stats_file` | Runtime or checkpoint option consumed by this adapter. |
...
| `hf_home` | Runtime or checkpoint option consumed by this adapter. |
| `hf_hub_cache` | Runtime or checkpoint option consumed by this adapter. |
| `hf_offline` | Runtime or checkpoint option consumed by this adapter. |
| `local_files_only` | Runtime or checkpoint option consumed by this adapter. |
| `allow_default_robot_dims` | Runtime or checkpoint option consumed by this adapter. |

Frequently used environment variables detected in the adapter scripts:

| Variable | Notes |
|---|---|
| `DEXORA_CKPT_PATH` | Optional override used by the local scripts or upstream runtime. |
| `DEXORA_CONFIG_PATH` | Optional override used by the local scripts or upstream runtime. |
| `DEXORA_ROOT` | Optional override used by the local scripts or upstream runtime. |
| `DEXORA_SIGLIP` | Optional override used by the local scripts or upstream runtime. |
| `DEXORA_T5` | Optional override used by the local scripts or upstream runtime. |
| `HF_HOME` | Optional override used by the local scripts or upstream runtime. |
| `HF_HUB_CACHE` | Optional override used by the local scripts or upstream runtime. |
