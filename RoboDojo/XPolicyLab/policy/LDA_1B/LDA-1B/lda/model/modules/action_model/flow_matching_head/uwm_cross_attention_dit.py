...
class AdaLayerNorm(nn.Module):
    def __init__(
        ...
        output_dim = embedding_dim // 2 * 3
        # output_dim = embedding_dim * 2
        self.silu = nn.SiLU()
        self.linear = nn.Linear(embedding_dim, output_dim)
        self.norm = nn.LayerNorm(embedding_dim // 4, norm_eps, norm_elementwise_affine)
    ...
    def forward(
        ...
        scale, shift, gate_self_attn, scale_ffn, shift_ffn, gate_ffn = temb.chunk(6, dim=1)
        x = self.norm(x) * (1 + scale[:, None]) + shift[:, None]
        return x, gate_self_attn, scale_ffn, shift_ffn, gate_ffn
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        if norm_type == "ada_norm":
            self.norm1 = AdaLayerNorm(dim * 4) # * 4 if has state else 3
        else:
    ...
    def forward(
        ...
        if self.norm_type == "ada_norm":
            norm_hidden_states, gate_self_attn, scale_ffn, shift_ffn, gate_ffn = self.norm1(hidden_states, temb)
        else:
        ...
        hidden_states = gate_self_attn[:, None] * attn_output + hidden_states
        ...
        ff_output = self.ff(
            norm_hidden_states * (1 + scale_ffn[:, None]) + shift_ffn[:, None]
            )

        hidden_states = gate_ffn[:, None] * ff_output + hidden_states
        if hidden_states.ndim == 4:
            hidden_states = hidden_states.squeeze(1)
...
class DiT(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # )
        if use_img_denoise:
            self.timestep_encoder = DualTimestepEncoder(
                embedding_dim=self.inner_dim, compute_dtype=compute_dtype
            )
        else:
        ...
        self.use_img_denoise = use_img_denoise
        ...
        self.proj_out_1 = nn.Linear(self.inner_dim * 4, 2 * self.inner_dim) # *4 if has state
    ...
    def forward(
        ...
        # Encode timesteps
        if self.use_img_denoise and obs_timestep is not None:
            temb = self.timestep_encoder(timestep, obs_timestep)
        else:
            temb = self.timestep_encoder(timestep)

        temb = torch.cat([encoder_hidden_states, temb], dim=-1)
