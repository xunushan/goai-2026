"""
TransformerForDiffusion — 从 patch_policy 抽取的核心骨干（不改逻辑）。

来源：patch_policy/models/diffusion_policy/diffusion_policy.py
- TransformerForDiffusion: encoder(cond: time+obs patches) + decoder(action, causal attn + patch-aware memory mask)
- 这里去掉了 diffusers / gpt.py 等无关依赖，仅内联 generate_mask_matrix。
"""
import math
...
def generate_mask_matrix(npatch, nwindow):
    """Block-causal mask：每个 time-window 的 npatch 个 token 块内全连接，跨 window 严格因果。"""
