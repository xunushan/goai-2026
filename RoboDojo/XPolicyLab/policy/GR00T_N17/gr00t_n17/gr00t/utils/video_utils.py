...
def _extract_frames_by_indices_pyav(
    video_path: str, indices: list[int] | np.ndarray
) -> np.ndarray:
    """Extract frames at specific indices using PyAV sequential decode."""
    ...
    for i in indices:
        ...
        if idx in frames:
            ...
            raise ValueError(
                f"Unable to read frame index {idx} from video {video_path}"
            )
...
def get_frames_by_indices(
    ...
    if video_backend == "torchcodec":
        ...
        return _extract_frames_by_indices_pyav(video_path, indices)
    else:
...
def get_frames_by_timestamps(
    ...
    if video_backend == "torchcodec":
        ...
        info = _get_video_info_ffmpeg(video_path)
        frame_indices = np.round(np.asarray(timestamps) * info["fps"]).astype(int)
        return _extract_frames_by_indices_pyav(video_path, frame_indices)
