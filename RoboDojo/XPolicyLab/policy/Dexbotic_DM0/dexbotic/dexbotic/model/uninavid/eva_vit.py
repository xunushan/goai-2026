...
class EVAVisionTowerLavis(nn.Module): 
    def __init__(self, vision_tower, image_processor, args, use_checkpoint=False, drop_path_rate=0.0, delay_load=False, dtype=torch.float32):
        ...
        self.args = args
        self.use_checkpoint = use_checkpoint
        self.image_processor_name = self._resolve_local_path(image_processor)
        self.drop_path_rate = drop_path_rate
        self.patch_size = 14
        self.out_channel = 1408

        if self.image_processor_name is None:
            raise ValueError(
                "UniNaVid requires `image_processor` to be present in the checkpoint config."
            )

        self.image_processor = CLIPImageProcessor.from_pretrained(self.image_processor_name)
        self.vision_config = CLIPVisionConfig.from_pretrained(self.image_processor_name)
    ...
    def _resolve_local_path(self, path):
        if path is None or os.path.isabs(path):
            return path
        model_root = getattr(self.args, "_name_or_path", None) or getattr(
            self.args, "name_or_path", None
        )
        return os.path.join(model_root, path) if model_root else path
