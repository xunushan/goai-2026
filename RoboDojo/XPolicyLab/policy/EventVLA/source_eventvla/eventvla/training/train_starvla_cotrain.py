...
class VLAMTrainer(TrainerUtils):
    ...
    def prepare_training(self):
        ...
        # Ensure DeepSpeed has a concrete micro-batch size when using batch_sampler.
        if hasattr(self.accelerator, "state") and self.accelerator.state.deepspeed_plugin is not None:
            ds_plugin = self.accelerator.state.deepspeed_plugin
            ds_config = ds_plugin.deepspeed_config or {}
            micro_bs = int(self.config.datasets.vla_data.per_device_batch_size)
            if ds_config.get("train_micro_batch_size_per_gpu", None) is None:
                ds_config["train_micro_batch_size_per_gpu"] = micro_bs
                ds_plugin.deepspeed_config = ds_config
            if hasattr(ds_plugin, "train_micro_batch_size_per_gpu"):
                ds_plugin.train_micro_batch_size_per_gpu = micro_bs
    ...
    def _train_step(self, batch_vla, batch_vlm):
        ...
        with self.accelerator.accumulate(self.model):
            ...
            # Sequence-aware memory reset by batch slot when episode rolls over.
            ...
            if keyframe_loss is not None:
                log_dict["keyframe_loss"] = keyframe_loss.item()
            for metric_name in [
                "keyframe_accuracy",
                "keyframe_pred_rate",
                ...
                if metric_value is not None:
                    log_dict[metric_name] = metric_value.item()
        return log_dict
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_yaml", type=str, default="eventvla/config/training/starvla_cotrain_oxe.yaml", help="Path to YAML config")
