...
class TrainerUtils:
    def prune_old_checkpoints(self, checkpoint_dir, keep_recent_checkpoints=None, incoming_checkpoints=0):
        """Keep only the most recent checkpoint files matched by training step."""
        if keep_recent_checkpoints is None:
            keep_recent_checkpoints = getattr(self.config.trainer, "keep_recent_checkpoints", 2)
        if keep_recent_checkpoints is None:
            return

        keep_recent_checkpoints = int(keep_recent_checkpoints)
        incoming_checkpoints = max(int(incoming_checkpoints), 0)
        if keep_recent_checkpoints <= 0 or not os.path.isdir(checkpoint_dir):
            return
        max_existing_checkpoints = max(keep_recent_checkpoints - incoming_checkpoints, 0)

        checkpoint_pattern = re.compile(r"steps_(\d+)_(?:pytorch_model\.pt|model\.safetensors)$")
        ...
        for checkpoint_name in os.listdir(checkpoint_dir):
            ...
            if match is None:
                continue
            checkpoints_with_steps.append((int(match.group(1)), checkpoint_name))

        if len(checkpoints_with_steps) <= max_existing_checkpoints:
            return

        checkpoints_with_steps.sort(key=lambda item: item[0])
        if max_existing_checkpoints > 0:
            checkpoints_to_delete = checkpoints_with_steps[:-max_existing_checkpoints]
        else:
            checkpoints_to_delete = checkpoints_with_steps
        for _, checkpoint_name in checkpoints_to_delete:
            checkpoint_path = os.path.join(checkpoint_dir, checkpoint_name)
            try:
                os.remove(checkpoint_path)
                if hasattr(self, "accelerator"):
                    self.accelerator.print(f"🧹 Removed old checkpoint: {checkpoint_path}")
                        ...
                        for key, value in sub_state_dict.items():
                            ...
                            if module_state[key].shape != value.shape:
                                skipped_shape_keys.append(
                                    f"{path}.{key}: ckpt={tuple(value.shape)} model={tuple(module_state[key].shape)}"
                                )
                                continue
                            filtered_sub_state[key] = value
                        module.load_state_dict(filtered_sub_state, strict=False)
                        if dist.get_rank() == 0:
                            print(f"✅ parameters loaded to module '{path}'")
                            if skipped_shape_keys:
                                print(f"⚠️ skipped shape-mismatched keys: {skipped_shape_keys}")
    ...
    def _reset_dataloader(dataloader, epoch_counter):
        ...
        # 2. set new epoch for sampler / batch_sampler (distributed core)
        ...
        if hasattr(dataloader, "batch_sampler") and callable(getattr(dataloader.batch_sampler, "set_epoch", None)):
            dataloader.batch_sampler.set_epoch(epoch_counter)
