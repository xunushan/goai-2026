...
## EventVLA Co-Training Guide: VLA with VLM Data 🚀

This guide outlines the process for integrating VLM (Vision-Language Model) data to co-train the EventVLA (Vision-Language-Action) framework, enhancing its general visual and language understanding.
...
You can download our case dataset [LLaVA-OneVision-COCO](https://huggingface.co/datasets/EventVLA/LLaVA-OneVision-COCO).  
...
Register your dataset by adding it to the `data_dict` in [qwen_data_config.py](./eventvla/dataloader/qwenvl_llavajson/qwen_data_config.py#L17).
...
```bash
python eventvla/dataloader/vlm_datasets.py --config_yaml your_train_config.yaml
```
...
Use this for VLM-specific pre-training or fine-tuning.
  ...
  * **Script:** `eventvla/training/train_starvla_vlm.py`
...
This simultaneously trains the model on both robotics (VLA) and multi-modal (VLM) data.
  ...
  * **Script:** `eventvla/training/train_starvla_cotrain.py`
