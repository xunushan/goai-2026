...
2. Running the evaluation by launching services in both `eventvla` and `simpler_env` environments.  
...
- **eventvla environment**: runs the policy inference server.  
- **simpler_env environment**: runs the simulation eval code.  


### Step 0. Download offical checkpoint

1) Download the checkpoint:[Qwen3VL-GR00T-Bridge-RT-1](https://huggingface.co/EventVLA/Qwen3VL-GR00T-Bridge-RT-1)
...
```bash
python eventvla/dataloader/lerobot_datasets.py --config_yaml examples/SimplerEnv/train_files/starvla_cotrain_oxe.yaml
```
...
```bash
python eventvla/model/framework/QwenGR00T.py --config_yaml examples/SimplerEnv/train_files/starvla_cotrain_oxe.yaml
```
