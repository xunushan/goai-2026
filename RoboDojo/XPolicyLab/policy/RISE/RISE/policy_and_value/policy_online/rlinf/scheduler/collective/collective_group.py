        ...
        with self._lock:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if "EADDRINUSE" in error_msg or "address already in use" in error_msg.lower():
                        retry_count += 1
                        self._logger.warning(
                            f"Port conflict detected for group {self._group_info.group_name} (attempt {retry_count}/{max_retries}), retrying with a new port..."
                        )
                        # Reset master port info so we can try a new port
                        if self._rank == 0:
                            self._coll_manager.reset_master_port_info(self._group_info.group_name)
                        ...
                        # Re-raise non-port-related errors
                        raise

            # If we've exhausted all retries, raise the last error
            raise RuntimeError(
                f"Failed to initialize process group {self._group_info.group_name} after {max_retries} retries due to port conflicts. Last error: {last_error}"
            )
