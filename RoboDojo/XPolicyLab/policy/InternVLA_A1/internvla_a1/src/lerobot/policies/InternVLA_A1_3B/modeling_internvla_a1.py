...
COSMOS_PATH = os.environ.get("COSMOS_PATH")
Qwen3_2B_PATH = os.environ.get("QWEN3_2B_PATH")
...
class QwenA1(nn.Module):
    ...
    def __init__(self, config: QwenA1Config):
        ...
        if not os.path.exists(os.path.join(COSMOS_PATH, "encoder.jit")):
            logging.warning(f"Cosmos-Tokenizer-CI8x8 not found, downloading...")
            from huggingface_hub import snapshot_download
            snapshot_download(repo_id="nvidia/Cosmos-Tokenizer-CI8x8", local_dir=COSMOS_PATH)

        self.cosmos = ImageTokenizer(
            checkpoint_enc=os.path.join(COSMOS_PATH, "encoder.jit"), 
            checkpoint_dec=os.path.join(COSMOS_PATH, "decoder.jit"), 
        )
