...
class MultiThreadedDatasetBuilder(tfds.core.GeneratorBasedBuilder):
    ...
    def get_parse_fcn(self):
        return type(self).PARSE_FCN

    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        split_paths = self._split_paths()
        parse_fcn = self.get_parse_fcn()
        return {split: parse_fcn(paths=split_paths[split]) for split in split_paths}
...
def chunks(values, n):
    d, r = divmod(len(values), n)
    for i in range(n):
        start = (d + 1) * (i if i < r else r) + d * (0 if i < r else i - r)
        yield values[start:start + (d + 1 if i < r else d)]


def chunk_max(values, n, max_chunk_sum):
    out = []
    remaining = list(values)
    for _ in range(int(np.ceil(len(remaining) / max_chunk_sum))):
        out.append(list(chunks(remaining[:max_chunk_sum], n)))
        remaining = remaining[max_chunk_sum:]
    return out
