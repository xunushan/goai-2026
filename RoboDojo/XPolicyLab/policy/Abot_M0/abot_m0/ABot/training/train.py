...
class VLATrainer(TrainerUtils):
    ...
    def _calculate_total_batch_size(self):
        ...
        grad_acc = getattr(self.config.trainer, "gradient_accumulation_steps", 1)
        if self.accelerator.distributed_type != DistributedType.DEEPSPEED:
            grad_acc = self.accelerator.gradient_accumulation_steps
        return (
    ...
    def _train_accumulation_context(self):
        """DeepSpeed ZeRO-2 handles grad accumulation internally and cannot use no_sync."""
        if self.accelerator.distributed_type == DistributedType.DEEPSPEED:
            self.accelerator._do_sync()
            return nullcontext()
    ...
    def _train_step(self, batch_vla, batch_vlm=None):
        ...
        with self._train_accumulation_context():
