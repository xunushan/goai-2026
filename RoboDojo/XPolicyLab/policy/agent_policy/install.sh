...
if ! conda env list | awk '{print $1}' | grep -qx "${CONDA_ENV}"; then
    echo "[agent_policy] ERROR: conda env '${CONDA_ENV}' not found." >&2
    exit 1
fi
...
try:
    importlib.import_module("XPolicyLab.policy.agent_policy.model")
    print("  codex_agent  importable as XPolicyLab.policy.agent_policy.model")
except Exception as exc:  # noqa: BLE001
    ok = False
    print(f"  codex_agent  NOT IMPORTABLE ({exc})")

if not ok:
    print("[agent_policy] ERROR: missing dependencies in conda env.")
    print("[agent_policy] Fix the environment by hand; this script installs nothing.")
    raise SystemExit(1)
print("[agent_policy] deps OK")
PY
...
if [[ -n "${bridge_url}" ]]; then
    echo "[agent_policy] CODEX_BRIDGE_URL=${bridge_url}"
else
    echo "[agent_policy] CODEX_BRIDGE_URL unset; deploy.yml's bridge_url will be used."
    echo "[agent_policy]   From the Mac, keep a tunnel up: tools/tunnel_mac.sh"
fi

echo "[agent_policy] Done. use conda env ${CONDA_ENV}"
