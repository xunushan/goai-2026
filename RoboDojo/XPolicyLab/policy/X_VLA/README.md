...
- Temporal ensemble (`temporal_ensemble_coeff` finite and `actions_per_chunk < temporal_ensemble_horizon`) re-plans once per `actions_per_chunk` steps and returns that many aligned ensembled actions per request (a sliding window over overlapping predictions), unlike the default chunked execution. It mirrors `act_lerobot`'s `_ServerTemporalEnsembler` / LeRobot's `ACTTemporalEnsembler`, operating on the raw model action chunk before rotate6d→quaternion conversion and gripper post-processing. When `actions_per_chunk == temporal_ensemble_horizon` (e.g. 30), consecutive re-plans never overlap and the ensemble is bypassed entirely.
