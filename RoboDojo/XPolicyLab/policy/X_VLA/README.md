...
| `temporal_ensemble_coeff` | Exponential weighting coefficient for online ACT temporal ensemble. `null` (default) disables it and executes the whole chunk; a finite value enables it and **requires `actions_per_chunk: 1`** (one aligned ensembled action per observation). Positive values weight older predictions more (LeRobot default 0.01). |
...
- Temporal ensemble (`temporal_ensemble_coeff` finite) re-plans every simulator step and returns one ensembled action per request, unlike the default chunked execution. It mirrors `act_lerobot`'s `_ServerTemporalEnsembler` / LeRobot's `ACTTemporalEnsembler`, operating on the raw model action chunk before rotate6d→quaternion conversion and gripper post-processing.
