...
class CasualWorldActionTransformer(
    ...
    def __init__(
        ...
        # Separate state encoder (defaults to same dim as action; overridden by trainer when state_dim != action_dim)
        ...
        # Placeholder: condition_embedder_action will be initialized in trainer
        # via deepcopy from the pretrained condition_embedder to avoid meta-tensor issues.
        self.condition_embedder_action = None
    ...
    def _forward_inference(
        ...
        # 1. Handle LoRA scaling, matching training
        ...
        # 2. Prepare video dimension information
        # noisy_latents current Input
        ...
        # 3. Robot data encoding (State & Action)
        # Note: action 0 or
        action_states = self.action_encoder(action) 
        state_states = self.state_encoder(state)
        ...
        # 4. Get rotary position embeddings (RoPE)
        # Compute the original RoPE here first, then concatenate in interleaved order
        rotary_emb = self.rope(hidden_states) 
        extra_rotary_emb = self.action_rope(extra_states) 

        # 5. Patchify video data
        hidden_states = self.patch_embedding(hidden_states.to(torch.bfloat16))
        hidden_states = hidden_states.flatten(2).transpose(1, 2)

        # 6. Core: rearrange tokens
        ...
        # 7. timeandtext (independent action embedder)
        s_r_end = num_state_tokens + num_ref_tokens
        action_end = s_r_end + num_action_tokens

        if timestep.ndim == 2:
            ts_seq_len = timestep.shape[1]
            video_ts = torch.cat([timestep[:, :s_r_end], timestep[:, action_end:]], dim=1)
            action_ts = timestep[:, s_r_end:action_end]
            video_ts_flat = video_ts.flatten()
            action_ts_flat = action_ts.flatten()
            video_seq_len = video_ts.shape[1]
            action_seq_len = action_ts.shape[1]
        else:
            video_seq_len = None
            action_seq_len = None
            video_ts_flat = timestep
            action_ts_flat = timestep

        encoder_hidden_states_raw = encoder_hidden_states  # save before text_embedder projection
        temb, video_timestep_proj, encoder_hidden_states, encoder_hidden_states_image = self.condition_embedder(
            video_ts_flat, encoder_hidden_states, encoder_hidden_states_image,
            timestep_seq_len=video_seq_len,
        )
        action_temb, action_timestep_proj, _, _ = self.condition_embedder_action(
            action_ts_flat, encoder_hidden_states_raw, None,
            timestep_seq_len=action_seq_len,
        )
        ...
        if timestep.ndim == 2:
            ...
            timestep_proj = video_timestep_proj

        if encoder_hidden_states_image is not None:
            encoder_hidden_states = torch.concat([encoder_hidden_states_image, encoder_hidden_states], dim=1)

        # 8. Build inference mask (preserve causal behavior)
        L = hidden_states.shape[1]
        device = hidden_states.device
        mask = torch.zeros((L, L), device=device)
        
        # Limit attention: past tokens cannot see future tokens
        ...
        # Extract and restore video latents
        # Concatenate the Ref part and denoised future part again
        ...
        # Unpatchify Restore to [B, C, T, H, W]
        ...
        return Transformer2DModelOutput(sample=output) # Note: to also get action, usually handle it at the sampler level
    ...
    def _forward_inference_action_only(
        ...
        return Transformer2DModelOutput(sample=None) # Note: to also get action, usually handle it at the sampler level
    ...
    def _forward_train(
        ...
        action_states = self.action_encoder(action)
        state_states = self.state_encoder(state)
        if getattr(self, '_freeze_action', False):
            action_states = action_states.detach()
            state_states = state_states.detach()
        num_action_tokens = action_states.shape[1]
        num_state_tokens = state_states.shape[1]
        num_ref_tokens = post_patch_width * post_patch_height
        num_noisy_tokens = (post_patch_num_frames - 1) * post_patch_width * post_patch_height
        ...
        # Split timestep into video and action parts for independent embedders
        s_r_end = num_state_tokens + num_ref_tokens
        action_end = s_r_end + num_action_tokens

        if timestep.ndim == 2:
            ts_seq_len = timestep.shape[1]
            # Split: [state | ref | action | noisy_video] -> video part & action part
            video_ts = torch.cat([timestep[:, :s_r_end], timestep[:, action_end:]], dim=1)  # state + ref + noisy
            action_ts = timestep[:, s_r_end:action_end]  # action only
        ...
        # Video tokens use the original condition_embedder
        encoder_hidden_states_raw = encoder_hidden_states  # save before text_embedder projection
        temb, video_timestep_proj, encoder_hidden_states, encoder_hidden_states_image = self.condition_embedder(
            video_ts_flat, encoder_hidden_states, encoder_hidden_states_image,
            timestep_seq_len=video_seq_len,
        )
        # Action tokens use the independent condition_embedder_action
        action_temb, action_timestep_proj, _, _ = self.condition_embedder_action(
            action_ts_flat, encoder_hidden_states_raw, None,
            timestep_seq_len=action_seq_len,
        )
        ...
        if timestep.ndim == 2:
            video_timestep_proj = video_timestep_proj.unflatten(2, (6, -1))   # (bs, video_seq, 6, D)
            action_timestep_proj = action_timestep_proj.unflatten(2, (6, -1)) # (bs, action_seq, 6, D)
            # Reassemble: [state | ref | action | noisy_video]
            timestep_proj = torch.cat([
                video_timestep_proj[:, :s_r_end],       # state + ref
                action_timestep_proj,                    # action
                video_timestep_proj[:, s_r_end:],        # noisy_video
            ], dim=1)
            # Also reassemble temb for output norm
            temb = torch.cat([
                temb[:, :s_r_end],
                action_temb,
                temb[:, s_r_end:],
            ...
            video_timestep_proj = video_timestep_proj.unflatten(1, (6, -1))
            action_timestep_proj = action_timestep_proj.unflatten(1, (6, -1))
            # When not per-token, use video temb as the shared temb (action temb used separately below)
            timestep_proj = video_timestep_proj
