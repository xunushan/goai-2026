#!/usr/bin/env python3
"""Validate an xvla_lerobot WebSocket server before Isaac Sim evaluation."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/xvla_lerobot_mock.json"),
    )
...
def validate_chunk(chunk: np.ndarray) -> dict[str, Any]:
    ...
    if np.any((grippers < 0.0) | (grippers > 1.0)):
        raise ValueError(f"Continuous grippers must stay in [0,1]: {grippers}")
    return {
...
async def run(args: argparse.Namespace) -> None:
    ...
    client = PolicyEvalClient(
        PolicyEvalClientConfig(
            url=args.url,
            evaluation_id="xvla-lerobot-mock",
        )
    )
    await client.connect(handshake=True)
    await client.reset(trial_id="xvla-lerobot-mock")
