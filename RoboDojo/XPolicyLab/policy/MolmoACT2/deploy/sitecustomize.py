"""MolmoACT2 eval client: extend ModelClient socket timeout for slow VLM inference."""
...
_TIMEOUT = float(os.environ.get("MOLMOACT_MODEL_CLIENT_TIMEOUT", "600"))
...
def _patch_model_client(module) -> None:
    ...
    if model_client_cls is None or getattr(model_client_cls, "_molmoact_timeout_patch", False):
        return
    ...
    model_client_cls._molmoact_timeout_patch = True
    _PATCHED = True


def _molmoact_import(name, globals=None, locals=None, fromlist=(), level=0):
...
builtins.__import__ = _molmoact_import
