...
class JointModel(nn.Module):
    def __init__(self, config):
        ...
        for mixture_name, mixture_config in config.mixture.items():
            ...
            mixture_config = OmegaConf.merge(config, mixture_config)
            mixture_config_dict = OmegaConf.to_container(mixture_config, resolve=True)
            mixture_config = MixtureConfig(**mixture_config_dict)
    ...
    def forward(
        ...
        # Prefix only (cache building)
        if inputs_embeds[1] is None:
            prefix_output = self.mixtures.vlm.forward(
        ...
        # Suffix only (decode with cache)
        elif inputs_embeds[0] is None:
            suffix_output = self.mixtures.action.forward(
            ...
            models = [self.mixtures.vlm, self.mixtures.action]
            num_layers = self.num_hidden_layers

            use_gradient_checkpointing = (
                hasattr(self.mixtures.action, "gradient_checkpointing")
                and self.mixtures.action.gradient_checkpointing
                and self.training
            ) or (hasattr(self, "gradient_checkpointing") and self.gradient_checkpointing and self.training)

            if self.training and hasattr(self.mixtures.action, "gradient_checkpointing"):
                if not self.mixtures.action.gradient_checkpointing:
                    self.mixtures.action.gradient_checkpointing = True
            ...
            def compute_layer_complete(layer_idx, inputs_embeds, attention_mask, position_ids, adarms_cond):
                ...
                cos, sin = self.mixtures.vlm.rotary_emb(dummy_tensor, position_ids)
                query_states, key_states = apply_rotary_pos_emb(
                    query_states, key_states, cos, sin, unsqueeze_dim=1
                )

                batch_size = query_states.shape[0]
                scaling = self.mixtures.vlm.layers[layer_idx].self_attn.scaling

                att_output, _ = eager_attention_forward(
                    self.mixtures.vlm.layers[layer_idx].self_attn,
                    query_states,
                    key_states,
                    value_states,
                    attention_mask,
                ...
                head_dim = self.mixtures.vlm.layers[layer_idx].self_attn.head_dim
